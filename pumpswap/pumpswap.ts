...
export class PumpSwapSDK {
  ...
  public async buy(mint: PublicKey, user: Keypair, solToBuy: number) {
    ...
    const pumpswap_buy_tx = await this.createBuyInstruction(pool, user.publicKey, mint, bought_token_amount, BigInt(Math.floor(solToBuy * (1 + slippage) * LAMPORTS_PER_SOL)));
  ...
  public async sell_exactAmount(mint: PublicKey, user: Keypair, tokenAmount: number) {
    ...
    const pool = await getPumpSwapPool(mint);
    const pumpswap_buy_tx = await this.createSellInstruction(await getPumpSwapPool(mint), user.publicKey, mint, BigInt(Math.floor(sell_token_amount * 10 ** 6)), BigInt(0));
    const ata = getAssociatedTokenAddressSync(mint, user.publicKey);
    const ix_list: any[] = [
      ...[
        ComputeBudgetProgram.setComputeUnitLimit({
  ...
  public async sell_percentage(mint: PublicKey, user: Keypair, percentage_to_sell: number) {
    const holding_token_amount = await getSPLBalance(solanaConnection, mint, user.publicKey);
    ...
    const pool = await getPumpSwapPool(mint);
    const pumpswap_buy_tx = await this.createSellInstruction(pool, user.publicKey, mint, BigInt(Math.floor(sell_token_amount * 10 ** 6)), BigInt(0));
    const ata = getAssociatedTokenAddressSync(mint, user.publicKey);
    const ix_list: any[] = [
      ...[
        ComputeBudgetProgram.setComputeUnitLimit({
    ...
    //sendNozomiTx(ix_list, wallet_1, latestBlockhash, "PumpSwap", "sell");
    // sendBundle(false, latestBlockhash.blockhash, transaction, pool, wallet_1)
    executeJitoTx([transaction], user, 'processed', latestBlockhash)
  }
